...
class AlarmServer:
    ...
    def _notify_updates(self):
        """/usr/local/lib/python3.13/threading.py:1043: RuntimeWarning: coroutine 'AlarmServerCoordinator._async_request_refresh' was never awaited self._invoke_excepthook(self)"""
    ...
    def initialize_sensors(self):
        ...
        _LOGGER.warn(f'inicializando os sensores da central#################################################################')
    ...
    def setEletrificadorStatus(self,eletrificador_byte):
        if '00'  in f'{eletrificador_byte:0>2X}':
           ...
           ele_state=STATE_OFF
           dados_evento = {
              "estado": AlarmControlPanelState.TRIGGERED,
              "dispositivo": "eletrificador",
              "timestamp": datetime.now().isoformat()
           }
    ...
    def handle_client(self):
        ...
        while self.running:
            ...
            if (time.time() - self.t) >35:
                ...
                _LOGGER.warn("Envia pedido de status 01")
    async def process_central(self, data):
      # Dicionário de modelos para simplificar o código
      MODELOS = {
        'A0': {'nome': 'Active-32 Duo', 'pgms': 4, 'eletrificador': 1, 'particoes': 4, 'zonas': 32},
        'A1': {'nome': 'Active 20 Ultra/GPRS', 'pgms': 4, 'eletrificador': 1, 'particoes': 2, 'zonas': 22},
        'A2': {'nome': 'Active 8 Ultra', 'pgms': 0, 'eletrificador': 0, 'particoes': 2, 'zonas': 12},
        'A3': {'nome': 'Active 20 Ethernet', 'pgms': 4, 'eletrificador': 1, 'particoes': 2, 'zonas': 22},
        'A4': {'nome': 'Active 100 Bus', 'pgms': 16, 'eletrificador': 1, 'particoes': 16, 'zonas': 99},
        'A5': {'nome': 'Active 20 Bus', 'pgms': 16, 'eletrificador': 1, 'particoes': 2, 'zonas': 32},
        'A6': {'nome': 'Active Full 32', 'pgms': 16, 'eletrificador': 0, 'particoes': 4, 'zonas': 32},
        'A7': {'nome': 'Active 20', 'pgms': 4, 'eletrificador': 1, 'particoes': 2, 'zonas': 32},
        'A8': {'nome': 'Active 8W', 'pgms': 4, 'eletrificador': 1, 'particoes': 2, 'zonas': 32},
        '4B': {'nome': 'M-300+', 'pgms': 2, 'eletrificador': 0, 'particoes': 0, 'zonas': 0},
        '5D': {'nome': 'm-300 Flex', 'pgms': 2, 'eletrificador': 0, 'particoes': 0, 'zonas': 0},
      }

      # Identificar o modelo
      modelo_codigo = f'{data[41]:0>2X}'
      modelo_info = MODELOS.get(modelo_codigo, None)
      _LOGGER.info(modelo_info)
      if modelo_info:
        self._model = modelo_info['nome']
        self._num_pgms = modelo_info['pgms']
        self.eletrificador = modelo_info['eletrificador']
        self._num_particoes = modelo_info['particoes']
        self._num_zonas = modelo_info['zonas']
    
      # Processar informações da central
      self.mac = data[29:41].decode("utf-8")
      self.ns = data[4:14].decode('ascii')
      # Processar versão do software
      vm = chr(data[42])
      vs = chr(data[43])
      vx = chr(data[44])
    
      self.particoes = ord(chr(data[51]))
      self.eletrificador = 0 if '00' in f'{data[54]:0>2X}' else 1
    
      # Montar versão do software
      if vx == '0':
        self._sw_version = f"{vm}.{vs if vs != '0' else '0'}"
      else:
        self._sw_version = f"{vm}.{vs}.{vx}"

      try:
        # Usar o registry de forma assíncrona
        device_registry = dr.async_get(self.hass)
        device = await self.hass.async_add_executor_job(
            device_registry.async_get_or_create,
            config_entry_id=self._config_entry_id,
            identifiers={(DOMAIN, self._unique_id)},
            name="Central de Alarme {self.mac}",
            manufacturer='JFL',
            model=self._model,
        ...
        if device:
            _LOGGER.info(f"Updated device {device.id} with model: {self._model}, SW version: {self._sw_version}")
          
            
            # Disparar evento de forma assíncrona
            await self.hass.bus.async_fire("device_updated", {
                "device_id": device.id,
                "updated_info": {self._sw_version}
            })
            
            # Inicializar componentes
            await self.initialize_zonas()
            await self.initialize_pgms()
            await self.initialize_sensors()
            await self.initialize_particoes()
            await self.initialize_eletrificador()
            
            # Disparar evento final
            await self.hass.bus.async_fire("alarm_central_updated")
        else:
            print("Device not found in the registry.")
    ...
    def process_evento(self,evento):
       ...
       if evento == "3441" :
          self.armed_away = False
          self.status_alarm=AlarmControlPanelState.ARMED_HOME
       ...
       if evento == '3401' or evento == '3407' or evento =='3403' or evento =='3404' or evento =='3408' or evento=='3409' :
          ...
          self.armed_night = False
          self.armed_home = False
          self.status_alarm=AlarmControlPanelState.ARMED_AWAY
       ...
       if evento == '1401' or evento =='1407' or evento =='1403' or evento=='1409':
          ...
          if evento == '1407':
            ...
            self.fire_alarm = False
            self._attr_state = AlarmControlPanelState.DISARMED
            self.status_alarm=AlarmControlPanelState.DISARMED
          dispatcher_send(self.hass, SIGNAL_PANEL_MESSAGE, self)    
       if evento == '1130' and self.armed_home == True:
          self.fire_alarm=True
          self.status_alarm=AlarmControlPanelState.TRIGGERED
          dispatcher_send(self.hass, SIGNAL_PANEL_MESSAGE, self)
       if evento == '3130':
          self.fire_alarm=False
          self.status_alarm=AlarmControlPanelState.DISARMED
    ...
    def process_data(self, data):
        ...
        _LOGGER.info("tamanho do Pacote %s",len(data))
        if len(data)==5:
           message = b'\x7B\6\x01\x40\x01'
           check = self.checksum(message)
           message += check.to_bytes(1,'big')
           self.client.send(bytes(message))
           _LOGGER.info("enviado ack da conexao %s",len(data))
        if len(data) ==24:
        ...
        while self.running:
            if self.client:
               ...
               if (time.time() - self.t) >35:
                  self.t=time.time()
                  self.send_pedido_Status()
                  _LOGGER.warn("Envia pedido de status  ' tempo")
               else:
    ...
    def turn_on_switch(self,pgm):
        if self.client:
           ...
           message = b'\x7b\6\x01\x50'+ str(pgm).encode()
           check = self.checksum(message)
           message += check.to_bytes(1,'big')
           self.client.send(bytes(message))
    ...
    def turn_off_switch(self,pgm):
        if self.client:
           self.comand=True
           _LOGGER.warn(f'acionando pgm:{pgm}')
           message = b'\x7b\6\x01\x51'+ str(pgm).encode()
           check = self.checksum(message)
           message += check.to_bytes(1,'big')
           self.client.send(bytes(message))
