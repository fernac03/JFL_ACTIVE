                     ...
                     try:
                      ...
                      if len(data) == 0:
                        ...
                        if chr(data[0]) == '{':
                           if chr(data[3]) == '$':
                              if data[21]== 0x01:
                                 _LOGGER.warn("desarmada, pronta e sem disparo.")
                              if data[21]== 0x02:
                                 _LOGGER.warn("armada, sem disparo.")
                              if data[21]== 0x03:
                                 _LOGGER.warn("armada STAY, sem disparo.")
                              if data[21]== 0x04:
                                 _LOGGER.warn("desarmada, não pronta e sem disparo.")
                              if data[21]== 0x81:
                              ...
                              #_LOGGER.warn("contador %s",f'{data[17:20]:0>2X}')
                              message2 = b'\x7b\x0a\x01\x24\x01'
                              message2 += bytes({data[17]})
                              message2 += bytes({data[18]})
                              message2 += bytes({data[19]})
                              message2 += bytes({data[20]})
                              check = self.checksum(message2)
                              message2 += check.to_bytes(1,'big')
                              message =b'\x7b\x0a\x01\x24\x01\x00\x00\x00\x0c\x59'
                              _LOGGER.warn('Send aknologe ')
                              _LOGGER.warn(message2)
                              conn.send(bytes(message2))
                             
                           message = b'\x7B\0x07\x01\x21\x01\x01'
                           check = self.checksum(message)
                           message += check.to_bytes(1,'big')
                           _LOGGER.warn('Send acept')
                        ...
                        if chr(data[0]) == '!':
                           ...
                           conn.send('+'.encode('ascii'))
                              ...
                              if time.time()-t>20:
                                ...
                                _LOGGER.warn("enviando Keep asc")
                        elif chr(data[0]) == '$':
